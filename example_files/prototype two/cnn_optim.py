...
def build_optimizer(model: torch.nn.Module, config: Dict[str, Any]) -> torch.optim.Optimizer:  # pragma: no cover
    return CnnOptim(config).build_optimizer(model)


def build_scheduler(
    optimizer: torch.optim.Optimizer, config: Dict[str, Any]
) -> Tuple[torch.optim.lr_scheduler._LRScheduler | None, str]:  # pragma: no cover
    return CnnOptim(config).build_scheduler(optimizer)
