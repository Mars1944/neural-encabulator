...
from cnn_model import SimpleCNN, MultiTaskCNN, count_parameters
...
class ModelManager:
    ...
    def _build_model(self) -> "torch.nn.Module":
        ...
        multitask = task.startswith("multi") or bool(self.config.get("multitask", False))
            ...
            if isinstance(cls_names, list) and len(cls_names) > 0:
                ...
                if build_class_index is not None and isinstance(tid, str) and tid.strip():
                    try:
                        ...
                        pass
            model = MultiTaskCNN(cfg) if multitask else SimpleCNN(cfg)
            return model
        ...
        model_cls = MultiTaskCNN if multitask else SimpleCNN
